...
// schema.
//
// The Go backend owns the schema. schema.sql is the frozen baseline.
// ApplySchema bootstraps a fresh database with it and no-ops when the schema
// already exists, so production is never re-DDL'd. Future schema changes are
// made directly here.
package db
...
// period. All other pool settings are pgx defaults; override via DSN query
// string if needed.
