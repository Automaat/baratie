...
//
// It is idempotent by presence check: if the `recipes` table already exists
// the schema is assumed complete and nothing runs — so an existing database
// is never re-DDL'd. Fresh databases (CI, dev, new installs) get the full
// schema in one batch.
func ApplySchema(ctx context.Context, pool *pgxpool.Pool) error {
	var exists bool
	if err := pool.QueryRow(ctx,
		`SELECT to_regclass('public.recipes') IS NOT NULL`,
	).Scan(&exists); err != nil {
