...
// initial password and display name.
func (h *Handler) CreateUser(w http.ResponseWriter, r *http.Request) {
	...
	user, err := h.store.Create(r.Context(), CreateParams{
		Username: username, PasswordHash: hash, Name: name, Surname: surname,
	})
	if err != nil {
...
// display name and surname — not username, password or admin flag.
func (h *Handler) UpdateUser(w http.ResponseWriter, r *http.Request) {
	...
	user, err := h.store.Update(r.Context(), id, UpdateParams{Name: name, Surname: surname})
	if err != nil {
		if errors.Is(err, ErrNotFound) {
